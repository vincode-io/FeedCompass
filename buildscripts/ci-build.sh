...
openssl aes-256-cbc -k "$ENCRYPTION_SECRET" -in buildscripts/certs/dist.cer.enc -d -a -out buildscripts/certs/dist.cer
openssl aes-256-cbc -k "$ENCRYPTION_SECRET" -in buildscripts/certs/dist.p12.enc -d -a -out buildscripts/certs/dist.p12
...
security import buildscripts/certs/dist.cer -k ~/Library/Keychains/github-build.keychain -A
security import buildscripts/certs/dist.p12 -k ~/Library/Keychains/github-build.keychain -P $KEY_SECRET -A
...
rm -f buildscripts/certs/dist.cer
rm -f buildscripts/certs/dist.p12
